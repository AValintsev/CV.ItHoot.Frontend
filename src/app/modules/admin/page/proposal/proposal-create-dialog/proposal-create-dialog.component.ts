...
export class ProposalCreateDialogComponent implements OnInit {
  ...
  click() {
    if (!this.proposal.resumes)
      this.proposal.resumes = []
  ...
  proposalBuildSelected() {
    this.resumeService.getAllResumesByProposalBuild(this.proposal.proposalBuild.id).subscribe(resumes =>{
