...
export class ProposalBuildDialogComponent implements OnInit {
  ...
  openPositionDialog(position: ProposalBuildPositionDto | null = null) {
    ...
    dialogRef.afterClosed().subscribe((position: ProposalBuildPositionDto) => {
      if (position == null)
        return;

      if (this.proposalBuild.positions.find(x=>x.positionName == position.positionName))
        return;
  ...
  canCreate():boolean {
    return this.proposalBuild && this.proposalBuild.projectTypeName != '' && this.proposalBuild.complexity && this.proposalBuild.complexityId > 0;
  }
