import {Component, Inject, OnInit} from '@angular/core';
import {MAT_DIALOG_DATA} from "@angular/material/dialog";

@Component({
  selector: 'cv-template-preview-dialog',
  templateUrl: './template-preview-dialog-admin.component.html',
  styleUrls: ['./template-preview-dialog-admin.component.scss']
})
export class TemplatePreviewDialogAdminComponent implements OnInit {

  resume: any = {
    id: 0,

    showLogo: true,
    resumeName: "Fullstack",
    isDraft: false,
    firstName: "Василь",
    lastName: "Марчук",
    position: {
      "positionId": 11,
      "positionName": "Fullstack Developer"
    },
    email: "vasil@example.com",
    site: "vas.com.ua",
    phone: "0968756532",
    code: "28000",
    country: "Україна",
    city: "Дніпро",
    street: "вулиця Перемоги 10",
    requiredPosition: "Fullstack",
    resumeTemplateId: 6,
    birthdate: "1985-10-14T21:00:00+00:00",
    picture: 'data:image/jpeg;base64,',
    aboutMe: "- Lorem ipsum dolor sit amet, consectetur adipiscing elit.\nVivamus a turpis at mi commodo luctus.\nPhasellus mattis nunc nec dui dictum, id feugiat orci tempor.\nMorbi malesuada dolor vel eros dictum, et aliquam ex mollis.\nSed finibus nisl eget arcu tincidunt semper.\n- Maecenas non elit a nisl eleifend vehicula at eget nisl.\nAliquam eget lorem in mi commodo interdum non eu augue.\nDonec tristique tellus vitae nulla ultricies luctus.\nUt vel libero sit amet sapien rhoncus condimentum non vel enim.\nVestibulum iaculis sem nec finibus mattis.\n- Mauris pulvinar velit eu vehicula interdum.\nCurabitur ac enim molestie, varius velit ut, egestas eros.",
    educations: [
      {
        id: 107,
        institutionName: "Вінницька академія ІТ",
        specialization: "інженер",
        degree: "магістр",
        description: "Vestibulum iaculis sem nec finibus mattis. Mauris pulvinar velit eu vehicula interdum. Curabitur ac enim molestie, varius velit ut, egestas eros.",
        startDate: "2019-12-10T22:00:00",
        endDate: "2020-11-11T22:00:00"
      },
      {
        id: 108,
        institutionName: "КПІ",
        specialization: "інженер",
        degree: "бакалавр",
        description: "Nulla venenatis nulla vitae sapien egestas, lobortis pretium enim pharetra. Cras congue orci vel augue vulputate sagittis. Mauris et justo posuere, rutrum urna sit amet, gravida dolor.",
        startDate: "2010-03-07T22:00:00",
        endDate: "2015-07-11T21:00:00"
      }
    ],
    experiences: [
      {
        id: 145,
        company: "ВЕБдев",
        position: "Fullstack Dev",
        description: "Donec tristique tellus vitae nulla ultricies luctus. Ut vel libero sit amet sapien rhoncus condimentum non vel enim. Vestibulum iaculis sem nec finibus mattis.",
        startDate: "2021-12-11T22:00:00",
        endDate: "2022-06-05T21:00:00"
      },
      {
        id: 146,
        company: "Smile",
        position: "Fullstack Developer",
        description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus a turpis at mi commodo luctus. Phasellus mattis nunc nec dui dictum, id feugiat orci tempor. Morbi malesuada dolor vel eros dictum, et aliquam ex mollis. Sed finibus nisl eget arcu tincidunt semper. Maecenas non elit a nisl eleifend vehicula at eget nisl. Aliquam eget lorem in mi commodo interdum non eu augue. Donec tristique tellus vitae nulla ultricies luctus. Ut vel libero sit amet sapien rhoncus condimentum non vel enim. Vestibulum iaculis sem nec finibus mattis. Mauris pulvinar velit eu vehicula interdum. Curabitur ac enim molestie, varius velit ut, egestas eros.",
        startDate: "2019-01-02T22:00:00",
        endDate: "2021-12-24T22:00:00"
      },
      {
        id: 147,
        company: "1С",
        position: "Fullstack Developer",
        description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus a turpis at mi commodo luctus. Phasellus mattis nunc nec dui dictum, id feugiat orci tempor. Morbi malesuada dolor vel eros dictum, et aliquam ex mollis. Sed finibus nisl eget arcu tincidunt semper. Maecenas non elit a nisl eleifend vehicula at eget nisl. Aliquam eget lorem in mi commodo interdum non eu augue. Donec tristique tellus vitae nulla ultricies luctus. Ut vel libero sit amet sapien rhoncus condimentum non vel enim. Vestibulum iaculis sem nec finibus mattis. Mauris pulvinar velit eu vehicula interdum. Curabitur ac enim molestie, varius velit ut, egestas eros.",
        startDate: "2018-05-04T21:00:00",
        endDate: "2020-12-18T22:00:00"
      }
    ],
    languages: [
      {
        id: 34,
        languageId: 3,
        languageName: "Ukrainian",
        level: 4
      },
      {
        id: 36,
        languageId: 5,
        languageName: "German",
        level: 3
      }
    ],
    skills: [
      {
        id: 93,
        skillId: 3,
        skillName: "oop",
        level: 3
      },
      {
        id: 94,
        skillId: 4,
        skillName: "sql",
        level: 1
      },
      {
        id: 95,
        skillId: 2,
        skillName: "jq",
        level: 2
      },
      {
        id: 96,
        skillId: 5,
        skillName: "php",
        level: 1
      },

    ],
    salaryRate: 0,
    availabilityStatus: 0,
    countDaysUnavailable: 0
  };


  constructor(@Inject(MAT_DIALOG_DATA) public id: number) {
    this.resume.resumeTemplateId = id;
  }

  ngOnInit(): void {
  }


}
